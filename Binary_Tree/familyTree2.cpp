...
public:
    ...
    familyMember *update(string name, int Nage, familyMember *temp) {
        if (temp == NULL || temp->name == name) {
        	
            return temp;
        } else if (name < temp->name) {
            return update(name, Nage, temp->left);
        } else {
            return update(name, Nage, temp->right);
        }
    }
...
int main() {
    ...
    while (true) {
        ...
        case 6: {
            ...
            cin>>NAge;
            familyMember *result = st.update(name, NAge, st.getRoot());
            if (result != NULL) {
                cout << " Age Updated! \n";
            } else {
                cout << "Member with name " << name << " not found.\n";
